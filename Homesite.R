setwd("D:\\Kaggle\\Homesite Quote Conversion\\")
trainMain <- read.csv("train.csv")
testMain <- read.csv("test.csv")
...
train$month <- as.integer(format(as.Date(train$Original_Quote_Date), "%m"))
train$year <- as.integer(format(as.Date(train$Original_Quote_Date), "%y"))
...
# seperating out the elements of the date column for the train set
test$month <- as.integer(format(as.Date(test$Original_Quote_Date), "%m"))
test$year <- as.integer(format(as.Date(test$Original_Quote_Date), "%y"))
...
write_csv(submission, "xgb_model4.csv")
